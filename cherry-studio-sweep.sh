...
#        同上一组目录，外加各 Partition 的 Cache/Code Cache
...
# 词可以写成 logs,crash 或 --only logs,crash：按逗号拆开逐个入列
...
if [ "${ASSUME_YES}" -eq 1 ]; then
  ...
  if ! have_filters; then
    echo "--yes 必须带筛选条件（如 --clean logs --yes）—— --all 不算筛选。" >&2
...
# Cherry Studio 是否正在运行（正在跑时清 GPU 缓存可能引发临时渲染异常）
app_running() {
...
if [ "${n_sel}" -eq 0 ]; then
  ...
  echo "   当前占用: $(du -sh "${CS_HOME}" 2>/dev/null | cut -f1)"
  echo
  echo "包含扫描在内，未删除任何文件。"
  exit 0
fi
...
printf '\n'
printf '   清理前占用: %s\n' "$(du -sh "${CS_HOME}" 2>/dev/null | cut -f1)"
